...
local main = "SUPER"
local workflow = require("lua.workflow")
...
local function run(keys, command, description, options)
	hl.bind(keys, hl.dsp.exec_cmd(command), described(options, description))
end
...
bind(main .. " + D", workflow.toggle_show_desktop, "Show or restore the desktop")
run(main .. " + SHIFT + D", "discord", "Open Discord")
run(main .. " + G", "systemctl suspend", "Suspend immediately")
...
bind(main .. " + H", workflow.minimize_active, "Minimize active window")
bind(main .. " + SHIFT + H", workflow.restore_last, "Restore last minimized window")

-- Scratchpad.
bind(main .. " + T", hl.dsp.workspace.toggle_special("scratchpad"), "Toggle scratchpad")
bind(
	main .. " + SHIFT + T",
...
-- Native Monocle stack navigation.
bind(main .. " + Left", hl.dsp.layout("cycleprev"), "Previous stacked window")
bind(main .. " + Right", hl.dsp.layout("cyclenext"), "Next stacked window")
bind(main .. " + Up", hl.dsp.layout("cycleprev"), "Previous stacked window")
bind(main .. " + Down", hl.dsp.layout("cyclenext"), "Next stacked window")

bind(main .. " + M", hl.dsp.window.fullscreen({ mode = "fullscreen", action = "toggle" }), "Toggle fullscreen")
bind(main .. " + SHIFT + M", hl.dsp.window.fullscreen({ mode = "maximized", action = "toggle" }), "Toggle maximize")
...
run("ALT + Tab", scripts .. "smart_alttab.sh", "Next window", { non_consuming = true })
run("ALT + SHIFT + Tab", scripts .. "smart_alttab.sh --reverse", "Previous window", { non_consuming = true })
run("ALT + Alt_L", scripts .. "alttab_modifier_release.sh", "Commit Alt-Tab", { release = true, locked = true })
run("ALT + Alt_R", scripts .. "alttab_modifier_release.sh", "Commit Alt-Tab", { release = true, locked = true })
...
for workspace = 1, 10 do
	...
	bind(main .. " + " .. key, hl.dsp.focus({ workspace = workspace }), "Open workspace " .. workspace)
	bind(
		main .. " + SHIFT + " .. key,
...
run(main .. " + Tab", scripts .. "workspace_switcher.sh", "Open workspace selector", { non_consuming = true })
run(
	main .. " + SHIFT + Tab",
...
bind(main .. " + ALT + Right", hl.dsp.group.next(), "Next grouped window")
bind(main .. " + ALT + Left", hl.dsp.group.prev(), "Previous grouped window")

-- Audio, media, and brightness.
local locked = { locked = true }
local repeat_locked = { locked = true, repeating = true }
local volume_up = "wpctl set-volume -l 1.0 @DEFAULT_AUDIO_SINK@ 2%+"
	.. " && wpctl set-mute @DEFAULT_AUDIO_SINK@ 0"
	.. " && "
	.. scripts
	.. "volume_osd.sh"
local volume_down = "wpctl set-volume -l 1.0 @DEFAULT_AUDIO_SINK@ 2%-"
	.. " && wpctl set-mute @DEFAULT_AUDIO_SINK@ 0"
	.. " && "
	.. scripts
	.. "volume_osd.sh"
local volume_mute = "wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle" .. " && " .. scripts .. "volume_osd.sh"
local brightness_up = "brightnessctl set 2%+ && " .. scripts .. "brightness_osd.sh"
local brightness_down = "brightnessctl set 2%- && " .. scripts .. "brightness_osd.sh"
...
run("XF86AudioMicMute", "wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle", "Toggle microphone mute", locked)
...
bind(main .. " + mouse:272", hl.dsp.window.drag(), "Drag active window", { mouse = true })
bind(main .. " + mouse:273", hl.dsp.window.resize(), "Resize active window", { mouse = true })

bind(main .. " + CTRL + H", hl.dsp.window.resize({ x = -40, y = 0, relative = true }), "Shrink window horizontally")
bind(main .. " + CTRL + L", hl.dsp.window.resize({ x = 40, y = 0, relative = true }), "Grow window horizontally")
bind(main .. " + CTRL + K", hl.dsp.window.resize({ x = 0, y = -40, relative = true }), "Shrink window vertically")
bind(main .. " + CTRL + J", hl.dsp.window.resize({ x = 0, y = 40, relative = true }), "Grow window vertically")
